...
from utils.stratified_split import train_test_split
...
class ECGTrainSet(Dataset):
    def __init__(self, feature_dict):
        ...
        self.label = feature_dict['labels']
...
def get_loader(loader_type, val_exists=True, feature_dict=None):
    ...
    if val_exists:
        if loader_type == "train":
            train_idx, _ = train_test_split(list(range(len(dataset))), dataset=dataset,
            ...
            _, temp_idx = train_test_split(list(range(len(dataset))), dataset=dataset,
                                        test_size=cfg.VAL_SPLIT+cfg.TEST_SPLIT,
                                        shuffle=True, random_state=cfg.RANDOM_SEED)
